...
| 2. 未救助明细排查 | 选择清算期，导入附件1和附件2，配置并执行筛查 | 先执行门诊重大疾病规则，再执行后续筛查并刷新统计 | 不参与导入和筛查 | 按导出未救助明细继续核对疑点数据 |
| 3. 应补应退明细排查 | 选择清算期，导入附件4和附件2，配置并执行独立筛查 | 先执行门诊重大疾病规则，再执行正负抵消等本台账规则 | 不参与导入和筛查 | 按导出应补应退明细继续核对应补应退数据 |
...
```mermaid
flowchart TD
    A["执行当前台账筛查"] --> B{"首项规则已启用？"}
    B -- 否 --> H["继续后续普通筛查规则"]
    B -- 是 --> C{"医疗类别命中配置？"}
    C -- 否 --> H
    C -- 是 --> D{"病种编码精确命中指定编码或已启用重大疾病编码库？"}
    D -- 否 --> H
    D -- 是 --> E["状态标记为拟通知2"]
    E --> F{"首项处理方式"}
    F -- 保留 --> G["标记未剔除并跳过后续规则"]
...
| 规则 | 结果 |
| :--- | :--- |
| 门诊重大疾病匹配（第一优先） | 医疗类别命中配置（默认 `门诊慢特病`、`造口袋门诊`），并且病种编码精确命中指定编码（默认 `M00500`）或重大疾病编码库中已启用的编码时，状态标记为 `拟通知2`；默认保留且不再执行后续规则，也可配置为剔除 |
| 医疗类别不在保留范围内 | 已剔除 |
| 医药机构名称命中剔除关键字 | 已剔除 |
...
4. 首项规则命中的记录是否统一为 `拟通知2`；配置“保留”时应为 `未剔除`，配置“剔除”时应为 `已剔除`。
...
应补应退明细有自己独立的筛查配置。它同样把“门诊重大疾病匹配”放在第一项，并额外包含：
...
1. 查看记录的筛查命中规则和备注。
2. 如果命中“门诊重大疾病匹配”，记录状态始终为 `拟通知2`，并直接跳过后续规则；处理方式为“保留”时不能被后续规则剔除。
