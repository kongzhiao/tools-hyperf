...
class NoticeImportJob extends AbstractJob
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            $reader->read($this->tempFile, function (array $row, int $rowIndex) use ($service, $period, $townMap, $sourceBatch, &$processed, $total, &$result, $logger) {
                $processed++;
                try {
                    $sequenceNo = $service->pickValue($row, ['序号', 'sequence_no']) ?: (string) ($rowIndex + 1);
                    ...
                    $data = [
                        ...
                        'priority_identity' => $service->pickValue($row, ['对象类别', '优先身份', '身份', 'priority_identity']) ?: null,
                        ...
                        'system_remark' => $service->pickValue($row, ['系统备注', '剔除备注', '备注', 'remark', 'system_remark']) ?: null,
                    ];
                    $data['calc_reimbursement_amount'] = $service->calcSupplementAmount($data);

                    $record = UnrescuedNoticeRecord::query()
                    ...
                    if ($record) {
                        ...
                        UnrescuedNoticeRecord::create($data);
                        $result['created']++;
                    }
                } catch (\Throwable $e) {
                    ...
                    $logger->warning('Notice import row failed: ' . $e->getMessage());
                }
                if ($processed % 500 === 0) {
            ...
            $logger->error('Notice import failed: ' . $e->getMessage(), ['uuid' => $this->uuid]);
            $this->failTask($e, '下放通知导入失败');
        } finally {
            if (file_exists($this->tempFile)) {
    ...
    private function finishImportTask(array $result): void
    {
        ...
        $title = $task ? $task->title : '下放通知_导入_通知明细_';
