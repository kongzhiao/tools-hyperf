...
- 第一项固定为 `outpatient_major_disease`：医疗类别命中配置，且病种编码精确命中指定编码或 `status=1` 的重大疾病编码库时，状态强制为拟通知2。
- 第一项默认 `action=keep`，命中后未剔除并跳过后续规则；改为 `exclude` 时已剔除但仍为拟通知2，同样跳过后续规则。
- 未命中第一项时恢复金额分档状态，再继续通用规则；重复筛查会刷新旧结果。
